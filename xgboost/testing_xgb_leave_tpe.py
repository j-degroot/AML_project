...
surrogate_model.load_model('XGB-TPE-noTPE.txt')
...
for i in data_X:
    pred = surrogate_model.predict(xgb.DMatrix(np.array(i).reshape(1,-1)))
...
ax.set_title('XGB surrogate predictions on leave-TPE out data')
ax.grid()
