...
for i in range(10):
    ...
    losses_smac = []
    losses_random = []
    with open('tpe-xgb-surrogate-runs/XGB_tpe'  + str(i) +'.csv', 'r') as read_obj:
    ...
    with open('smac-xgb-surrogate-runs/smac'  + str(i) +'.csv', 'r') as read_obj:
        csv_reader = reader(read_obj)
        header = next(csv_reader)
        # Check file as empty
        if header != None:
    ...
    with open('random-xgb-surrogate-runs/xgb_'  + str(i) +'.csv', 'r') as read_obj:
        csv_reader = reader(read_obj)
        header = next(csv_reader)
        # Check file as empty
        if header != None:
...
ax.plot(x_smac, y_smac, color = '#e76100', linestyle = 'solid', lw = 2.5, label= 'SMAC XGBOOST', path_effects=[pe.Stroke(linewidth=3.5, foreground='w'), pe.Normal()])

ax.plot(x_tpe, y_tpe, 'k--', lw = 2.3, label= 'TPE XGBOOST', path_effects=[pe.Stroke(linewidth=3.5, foreground='w'), pe.Normal()])

ax.plot(x_random, y_random, color = '#5D3C99', linestyle =  (0, (3, 1, 1, 1)), lw = 2.5, label= 'RANDOM SEARCH XGBOOST', path_effects=[pe.Stroke(linewidth=3.5, foreground='w'), pe.Normal()])
...
ax.set_title('Optimizers on XGBoost Surrogate')
ax.grid()
